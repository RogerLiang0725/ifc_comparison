...
- **Walls** are represented by the baseline extracted from the `Axis` representation of `IfcWall`, using the corresponding `IfcPolyline` in world coordinates. 
- **Openings** are represented by a baseline derived from the local bounding box of each `IfcOpeningElement`, converted to world coordinates through the opening placement matrix. 
- **Normalization** is performed by translating all extracted records into a shared local frame using the lower-left corner of the compared layout extent, which is consistent with the manuscript’s use of a unified coordinate system for IFC comparison. 
- **Host-aware checking** currently relies on the presence of `IfcRelVoidsElement` for opening-related validation. 
## Usage
